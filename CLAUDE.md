...
`cwsd` is a CW (Morse) sender daemon for ham radio transceivers (developed against the Icom IC-7300). It runs on Linux, links against **hamlib**, and exposes two independent network services so logging/contest software can control the rig and send Morse over the wire.
...
- Requires CMake ≥ 3.27, a C++17 compiler, and hamlib dev headers/libs (`target_link_libraries(cwsd hamlib)`).
...
- The two servers each spawn a thread in their constructor and join it in `stop()`. hamlib calls (rigctld) and serial ioctls (cwdaemon) are confined to their respective worker threads.
