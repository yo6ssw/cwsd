...
struct code_style {
  ...
  float keying_compensation_ms =
      0.0f;  // ms to add to all dits and dahs. the extra length is removed from
             // the spacing
  float character_wpm = 24.0f;
  float farnsworth_wpm = 24.0f;
  float paddle_switchpoint =
      50.0f;  // [10..90]. percentage of a dot. 50 is normal
