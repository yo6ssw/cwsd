...
float code_style::space_impulses_in_paris() {
    return static_cast<float>(NUMBER_OF_CHAR_SPACES_IN_PARIS * impulses_between_characters +
...
float map(float speed, float speed_min, float speed_max, float value_min, float value_max) {
...
//    weighting = map(character_wpm, speed_low, speed_high, weighting_low, weighting_high);
    ...
    speed.dit_ms = (SECONDS_IN_A_MINUTE / impulsesPerMinute) * MILLISECONDS_IN_A_SECOND;
    speed.dah_ms = speed.dit_ms * dah_dit_ratio;
    speed.space_between_chars_ms = speed.dit_ms * static_cast<float>(impulses_between_characters);
    speed.space_between_words_ms = speed.dit_ms * static_cast<float>(impulses_between_words);

    float on_ms_per_minute = non_space_impulses_in_paris()
                             * farnsworth_wpm
                             * speed.dit_ms;

    float off_ms_per_minute = (SECONDS_IN_A_MINUTE * MILLISECONDS_IN_A_SECOND) - on_ms_per_minute;
    float pause_impulse_ms = off_ms_per_minute / (space_impulses_in_paris() * farnsworth_wpm);

    speed.space_between_chars_ms = static_cast<float>(impulses_between_characters) * pause_impulse_ms;
    speed.space_between_words_ms = static_cast<float>(impulses_between_words) * pause_impulse_ms;
    ...
    speed.weighted_inter_element_space_ms = speed.dit_ms * static_cast<float>(100 - weighting) / 50.0f;
    ...
    // add letterspace
    speed.weighted_inter_element_space_ms += (3.0f * speed.weighted_dit_ms) * extra_letterspace_percent / 100.0f;

    return speed;
}

int ratio_to_int(float ratio) {
    return static_cast<int>(ratio * 50.0f / 3.0f);
}
