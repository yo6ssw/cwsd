...
enum class paddle_side { left, right };

enum class paddle_state : uint8_t { none = 0, dit = 1, dah = 2, both = 3 };

enum class mode_type { straight, iambic_a, iambic_b, tune };
...
struct ptt_settings {
  ...
  float tail_ms;
  uint8_t
      hang_type;  // 0 => no hang, 1 => wordspace + 1 dit, 2 => wordpsace + 2
                  // dits, 3 => wordspace + 3 dits, 4 => wordspace + 4 dits
  uint8_t reserved[3];
};
...
/**
 * returns the current size of the contents stored in the memory with specified
 * slot
 * @param slot
