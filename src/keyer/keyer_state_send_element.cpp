...
void state_send_element::enter() {
  ...
  time_to_exit_state =
      hardware->current_ms() + weighted_element_length_ms(data.sent_element);
  time_to_check_paddles =
      hardware->current_ms() +
      weighted_element_length_ms(element::dit) * 0.0 / 50.0f;
...
state_type state_send_element::update(uint32_t now_ms) {
  ...
  return now_ms >= time_to_exit_state ? state_type::inter_element_space
