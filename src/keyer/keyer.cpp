...
// Serializes the keyer's shared state across threads. The producer thread feeds
// the keyer via winkeyer_data()/get_speed() while the dedicated keyer thread
// consumes it in update(); both touch data.winkeyer_* , the winkeyer_buffer and
// the active profile's speed/timings. This mutex is non-recursive, so it is
// locked ONLY at these three entry points — the internal helpers they call
// (set_speed(), the state machine's update()s, etc.) must run unlocked.
...
static float audio_level_from_percentage(uint8_t percentage) {
  ...
  double a = 1e-3;
  // Note: in the above mentioned article, b is 6.908. However, exp(6.908) is
  // 1000.24475096 and that leads to clipping when multiplied by a = 1e-3,
  // because we'll get a multiplying factor of 1.00024
  double b = 6.9077;
  float volume;
  if (percentage == 0) {
    volume = 0;
...
uint8_t get_current_profile_id() { return current_profile; }

profile& get_current_profile() { return profiles[current_profile]; }
...
uint32_t element_length_ms(element e) {
  //        auto wpm = data.winkeyer_enabled ? data.winkeyer_speed_wpm :
  //        data.speed_wpm; return e == element::dit ? 1200 / wpm : (1200 *
  //        data.dah_dit_ratio) / wpm;

  auto& profile = get_current_profile();
  if (data.winkeyer_enabled) {
    return e == element::dit
               ? 1200 / data.winkeyer_speed_wpm
               : (1200 * profile.style.dah_dit_ratio) / data.winkeyer_speed_wpm;
  }
  return e == element::dit ? profile.timings.weighted_dit_ms
                           : profile.timings.weighted_dah_ms;
}

uint32_t weighted_element_length_ms(element e) {
  auto& profile = get_current_profile();
  if (data.winkeyer_enabled) {
    return static_cast<uint32_t>(element_length_ms(e) *
                                 profile.style.weighting / 50.0f);
  }
  return e == element::dit ? profile.timings.weighted_dit_ms
                           : profile.timings.weighted_dah_ms;
}
...
uint32_t inter_element_space_length_ms() {
  auto& profile = get_current_profile();
  if (data.winkeyer_enabled) {
    return static_cast<uint32_t>(element_length_ms(element::dit) *
                                 ((100 - profile.style.weighting) / 50.0f));
  }
...
void switch_to(state_type next_state) {
  if (data.current_state == nullptr ||
      next_state != data.current_state->type()) {
    //            printf("state: %s -> %s\n",
    //                   data.current_state == nullptr ? "<null>" :
    //                   get_state_name(data.current_state->type()),
...
bool persist_memories() { return true; }
...
void update() {
  ...
  // Drain zero-duration (routing) transitions within a single tick. States such
  // as winkeyer/play only pick the next element and return immediately;
  // switch_to() merely enter()s the next state, so without this loop the new
  // state's update() would not run until the following poll tick. That cost a
  // full poll interval of dead air on every inter-element gap. Timed states
  // (send_element, *_space, autospace) return their own type until their
  // deadline, which breaks the loop; the cap guards against any unexpected
  // routing cycle.
  constexpr int max_transitions_per_tick = 16;
  for (int i = 0; i < max_transitions_per_tick && data.current_state != nullptr;
       i++) {
...
void set_speed(float wpm) { get_current_profile().set_speed(wpm); }
...
void add_char_to_recording_memory(char c) {
  ...
  bool space_left =
      data.recording_index < sizeof(data.memory[data.recording_slot]);
  if (!space_left) {
    stop_recording();
  }
}
...
bool is_recording() { return data.is_recording; }

bool is_playing() { return data.is_playing; }
...
uint8_t count_memories() { return MemorySlots; }

uint16_t get_memory_max_size() { return MemorySlotSize; }
...
void winkeyer_data(uint8_t c) {
  ...
  if (!execute_pending_command &&
      data.winkeyer_remaining_buffered_arguments == 0 &&
      is_winkeyer_command(c) && !is_winkeyer_buffered_command(c)) {
  ...
  if (execute_pending_command) {
    ...
    switch (data.winkeyer_pending_command) {
      case 0x02:  // set speed
        //                    printf("Setting winkeyer speed to %d from
        //                    %f",data.winkeyer_arguments[0],
    ...
    if (data.winkeyer_remaining_buffered_arguments > 0) {
      ...
      if (is_winkeyer_command(c) && is_winkeyer_buffered_command(c)) {
        data.winkeyer_remaining_buffered_arguments =
            winkey_commands_parameter_count[c];
      }
    }
  }
}
...
uint32_t inter_word_space_length_ms() {
  if (data.winkeyer_enabled) {
    return element_length_ms(element::dit) *
           get_current_profile().style.impulses_between_words;
  }
...
bool save_profile(const profile& p, uint32_t& address) { return true; }

bool save_memory(const char* m, uint32_t& address) { return true; }

bool load_profile(profile& p, uint32_t& address) { return true; }

bool load_memory(char* m, uint32_t& address) { return true; }
...
void profile::update_timings() { timings = style.calculate_timings(); }
...
float profile::get_speed() const { return style.character_wpm; }
