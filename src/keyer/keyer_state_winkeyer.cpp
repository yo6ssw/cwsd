...
bool is_winkeyer_command(uint8_t c) { return c <= 0x1F; }

bool is_winkeyer_buffered_command(uint8_t c) { return c <= 0x1F && c >= 0x18; }

void state_winkeyer::enter() { data.winkeyer_enabled = true; }
...
state_type state_winkeyer::update(uint32_t elapsed_ms) {
  ...
  if (data.playing_bit == 0) {
    ...
    if (data.winkeyer_buffer.empty()) return state_type::winkeyer;
    ...
    if (is_winkeyer_command(c) &&
        data.winkeyer_buffer.size() <= winkey_commands_parameter_count[c]) {
      return state_type::winkeyer;
    }
    ...
    if (is_winkeyer_command(c)) {
      ...
      switch (c) {
        case 0x1C:  // buffered speed change
          //                        printf("Buffered speed change to %d. Current
          //                        winkeyer speed wpm is %f\n", arguments[0],
      ...
      if (c == '|') {
        ...
        if (pattern == nullptr) {
          //                        printf("!!! Pattern not found for char
          //                        [%c]\n", c);
