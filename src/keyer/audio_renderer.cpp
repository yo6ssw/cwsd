...
namespace keyer {
    ...
    void renderer::set_wave_type(wave_type type) {
        osc.set_wave_type(type);
    }
    ...
    void renderer::update_amp() {
        ...
        if (key_on && ramp_index < ramp_up_samples) {
            float alpha = static_cast<float>(ramp_index) / static_cast<float>(ramp_up_samples);
            amp = sinf(alpha * (float) M_PI / 2.0f); // 0 to PI/2
            ramp_index++;
        } else if (!key_on && ramp_index < ramp_down_samples) {
            float alpha = static_cast<float>(ramp_index) / static_cast<float>(ramp_down_samples);
...
//        right = volume * osc.get_at_phase_offset(0xffffffff >> 4);
    ...
    void renderer::set_level(float value) {
        level = value;
    }
