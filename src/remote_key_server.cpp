//
// Created for remote paddle keying. See remote_key_server.h for the design rationale.
//
...
namespace {
    // Local replay clock. CLOCK_MONOTONIC matches clock_nanosleep(TIMER_ABSTIME) below.
    uint64_t now_us() {
        timespec ts{};
    ...
    void sleep_until_us(uint64_t t_us) {
        ...
        // EINTR-restart is handled by passing TIMER_ABSTIME and re-sleeping to the same target.
        while (clock_nanosleep(CLOCK_MONOTONIC, TIMER_ABSTIME, &ts, nullptr) == EINTR) {
        }
    }

    // Same real-time promotion the keyer thread uses; replayed elements need it just as much.
    void try_set_realtime_priority() {
        ...
        if (pthread_setschedparam(pthread_self(), SCHED_FIFO, &sp) != 0) {
            LOG(WARNING) << "remote_key: could not enable real-time scheduling ("
                         << std::strerror(errno) << "); run as root or grant CAP_SYS_NICE";
        } else {
            LOG(INFO) << "remote_key: replay thread running at real-time priority " << sp.sched_priority;
        }
    }
}

remote_key_server::remote_key_server(remote_key_config c)
        : cfg(std::move(c)) {
...
remote_key_server::~remote_key_server() {
    stop();
}
...
void remote_key_server::receive_worker() {
    ...
    while (is_running) {
        ...
        // A new session (operator restart) or an explicit reset re-anchors and drops anything queued.
        ...
        if (new_session || (pkt.flags & remote_key::FLAG_SESSION_RESET)) {
            ...
            last_enqueued_src = 0;
            queue.clear();
            reset_requested.store(true, std::memory_order_relaxed);  // replay forces key-up
            LOG(INFO) << "remote_key: new keying session " << session_id;
        }

        for (const auto &e : pkt.edges) {
            // Dedup FIRST: last_enqueued_src is monotonic within a session and is only reset on a
            // genuine new session (above), never on a silence re-anchor. So an edge we already
            // played is always dropped here -- crucially, the stale history that keepalives keep
            // re-sending is never replayed after a silence gap re-anchors the playout clock.
            if (e.ts_us <= last_enqueued_src) {
                continue;   // already have it (history overlap / duplicate) -> idempotent
            }
            if (!have_anchor) {
                // Anchor on the first genuinely-new edge (not a stale history edge): map the
                // operator timeline to local time offset by the playout delay. Every subsequent
                // edge is scheduled relative to this pair, so spacing is preserved.
            ...
            if (scheduled < now) {
                // Arrived later than its playout slot: the buffer is too shallow for this link.
                // Play immediately rather than drop it; spacing slips but the element is not lost.
...
void remote_key_server::replay_worker() {
    ...
    if (!open_port()) {
        LOG(WARNING) << "remote_key: " << cfg.device << " not available yet; will retry";
    }
    ...
    const uint64_t max_down_us = static_cast<uint64_t>(cfg.max_key_down_ms) * 1000ull;
    const uint64_t silence_us  = static_cast<uint64_t>(cfg.silence_ms) * 1000ull;

    while (is_running) {
        uint64_t now = now_us();

        // Reconnect the serial device if it went away (USB unplug), roughly every 100 ms.
        ...
        if (last_rx_us.load(std::memory_order_relaxed) != 0 && since_rx > silence_us) {
        ...
        // --- apply all edges now due, and peek the next one for scheduling/PTT-lead ---
        ...
        uint8_t  next_state = 0;
        {
            ...
            while (!queue.empty() && queue.front().at_us <= now) {
                ...
                if (down && !ptt_is_on) {
                    set_ptt(true);   // lead window may have been missed; assert now at the latest
                    ptt_is_on = true;
                }
                if (down != key_is_down) {
                    set_key(down);
        ...
        // --- PTT sequencing: lead before the next key-down, tail after the last key-up ---
        ...
        if (ptt_is_on && !key_is_down && now >= ptt_release_at_us) {
            // Don't drop PTT if another key-down is already imminent (mid-character gap).
        ...
        // --- decide how long to sleep: until the next edge, but cap so watchdogs keep ticking ---
