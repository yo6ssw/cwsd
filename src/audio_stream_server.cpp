...
// by the raw Opus packet. The receiver uses the sequence to detect loss /
// reorder. Subscription (client -> server): any datagram sent to the bound port
// registers or refreshes the sender; its contents are ignored. Clients must
// keep sending (a small periodic keepalive) or they are dropped after
...
audio_stream_server::audio_stream_server(audio_stream_config cfg)
  ...
  // from the next packet (opus_decode with decode_fec=1). The encoder only
  // spends bits on FEC when it expects loss, so the loss-percent estimate must
  // be > 0.
  ...
  LOG(INFO) << "audio streaming " << config.sample_rate << "Hz/"
            << config.channels << "ch opus@" << config.bitrate << "bps"
...
bool audio_stream_server::open_socket() {
  ...
  if (bind(sockfd, reinterpret_cast<const sockaddr*>(&addr), sizeof(addr)) <
      0) {
    LOG(ERROR) << "audio: bind on port " << config.port
               << " failed: " << std::strerror(errno);
    return false;
  }

  // Non-blocking so draining subscription datagrams never stalls the capture
  // cadence.
...
bool audio_stream_server::open_capture() {
  ...
  int err =
      snd_pcm_open(&pcm, config.device.c_str(), SND_PCM_STREAM_CAPTURE, 0);
  if (err < 0) {
    pcm = nullptr;
    return false;
  }
...
void audio_stream_server::poll_clients() {
  ...
  // sender; the payload is irrelevant, so the smallest possible keepalive
  // works.
  ...
  while (true) {
    ...
    if (it == clients.end()) {
      LOG(INFO) << "audio: new subscriber " << inet_ntoa(from.sin_addr) << ":"
                << ntohs(from.sin_port) << " (" << clients.size() + 1
                << " total)";
  ...
  for (auto it = clients.begin(); it != clients.end();) {
    if (now - it->second.last_seen_ms >=
        static_cast<uint32_t>(config.client_timeout_ms)) {
      LOG(INFO) << "audio: dropping silent subscriber "
...
void audio_stream_server::broadcast(const uint8_t* payload, size_t len) {
  ...
  for (auto& entry : clients) {
    sendto(sockfd, packet, HEADER_BYTES + len, 0,
           reinterpret_cast<const sockaddr*>(&entry.second.addr),
           sizeof(entry.second.addr));
  }
}

void audio_stream_server::work() {
  std::vector<int16_t> pcm_buf(static_cast<size_t>(frame_size) *
                               config.channels);
  ...
  while (is_running) {
    if (pcm == nullptr) {
      if (!open_capture()) {
        if (capture_was_open) {
          LOG(WARNING) << "audio: capture device " << config.device
                       << " unavailable";
    ...
    if (frames < 0) {
      LOG(WARNING) << "audio: read failed (" << snd_strerror(frames)
                   << "), reopening device";
      close_capture();
      continue;
    }

    // One subscription/expiry sweep per frame (~every frame_ms) is plenty
    // responsive.
    poll_clients();

    auto encoded =
        opus_encode(encoder, pcm_buf.data(), static_cast<int>(frames), opus_buf,
                    sizeof(opus_buf));
    if (encoded < 0) {
