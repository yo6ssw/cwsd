...
namespace {
// Promote the calling thread to real-time scheduling and lock the process
// memory so the keyer's element timing is not stretched by ordinary scheduler
// preemption or page faults. Requires CAP_SYS_NICE / a suitable RLIMIT_MEMLOCK
// (granted by the systemd unit or root); on failure we log and keep running at
// normal priority rather than aborting.
void try_set_realtime_priority() {
  ...
  if (pthread_setschedparam(pthread_self(), SCHED_FIFO, &sp) != 0) {
    LOG(WARNING)
        << "cwdaemon: could not enable real-time scheduling for the keyer "
           "thread ("
        << std::strerror(errno)
        << "); run as root or grant CAP_SYS_NICE for jitter-free keying";
  } else {
    LOG(INFO) << "cwdaemon: keyer thread running at real-time priority "
              << sp.sched_priority;
  }
}
}  // namespace
...
uint32_t key_interface::current_ms() { return clock.ellapsed_ms(); }

bool key_interface::is_paddle_pressed(keyer::paddle_side side) { return false; }

void key_interface::on_key_down() { ioctl(fd, TIOCMBIS, &key); }

void key_interface::on_key_up() { ioctl(fd, TIOCMBIC, &key); }
...
void cwdaemon_server::client_worker(key_interface* iface, udp_server* server,
                                    timer* clock) {
  ...
  while (is_running) {
    if (server->receive()) {
      ...
      bool is_abort_send_cmd =
          message.size() > 1 && message[0] == 27 && message[1] == '4';
      bool is_exit_cmd =
          message.size() > 1 && message[0] == 27 && message[1] == '5';
      ...
      if (cw_daemon::is_tuning_command(wk_data)) {
        ...
        // which do not take a time parameter, so we need to remove it from the
        // stream after we use it to know when to stop the tuning.
...
void cwdaemon_server::update() {
  // Keying is driven by the dedicated keyer_worker() thread (see keyer_worker),
  // not by the shared main-loop cadence, so there is nothing to do here.
}

void cwdaemon_server::keyer_worker() {
  try_set_realtime_priority();

  // Open the serial control lines straight away rather than waiting for the
  // first throttled check below.
  iface->update();

  // Tick the keyer state machine at a fine, fixed cadence. The element key-up
  // edge can only land on a tick, so a tighter interval (here 1 ms) plus the
  // real-time priority above keeps element lengths close to their nominal
  // duration.
  int iface_check_divider = 0;
  while (is_running) {
    keyer::update();

    // Connect/disconnect detection (isatty + possible reopen) is comparatively
    // heavy and does not need the keyer's cadence; run it roughly every 100 ms.
...
cwdaemon_server::cwdaemon_server(std::string device, uint16_t listen_port,
  ...
  worker_thread =
      std::thread(&cwdaemon_server::client_worker, this, iface, server, &clock);
}
