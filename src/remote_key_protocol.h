...
//
// Real paddle keying cannot stream "an edge happened now" events: internet
// jitter would smear the spacing and destroy character formation. Instead the
// operator side timestamps every key transition against a per-session monotonic
// clock, and the rig side replays them behind a fixed playout delay (see
// remote_key_server). As long as a packet arrives before its scheduled playout
// time, the spacing between edges is reproduced exactly, independent of arrival
// jitter.
//
// Loss is dangerous here: a dropped "key up" edge would leave the transmitter
// keyed. Two defences live in the protocol; a third (hardware-style watchdog)
// lives in the replay engine:
//   1. Every packet carries the most recent edges (a short history), so a
//   single lost datagram is
//      recovered by the next one. Edges are idempotent: the receiver dedups by
//      source timestamp.
//   2. A packet with zero edges is a keepalive; the receiver forces a safe
//   (key-up) state if the
//      stream goes silent.
//
// Multi-byte fields are big-endian, matching audio_stream_server's on-the-wire
// convention.
//
...
// packet.flags bits
constexpr uint8_t FLAG_SESSION_RESET =
    0x01;  // operator restarted; drop anchor + force safe

// edge.state bits
constexpr uint8_t KEY_DOWN =
    0x01;  // bit set => key closed (transmitting an element)
constexpr uint8_t PTT_REQ =
    0x02;  // optional explicit PTT request (rig may derive its own)
...
struct packet {
  uint16_t session_id =
      0;  // random per operator session; a change re-anchors the rig
  uint8_t flags = 0;
...
// Keep history short enough to fit comfortably in one datagram while still
// bridging a few lost packets. At 40 WPM there are well under ~100
// edges/second, so even a handful of history edges covers tens of milliseconds
// of loss.
...
// Serialize a packet. Edges beyond MAX_EDGES are dropped (callers send the most
// recent ones).
...
// Parse a datagram into `out`. Returns false on a malformed/foreign packet
// (caller ignores it).
