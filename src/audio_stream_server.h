...
struct audio_stream_config {
  bool enabled = false;
  std::string device =
      "default";      // ALSA capture device (e.g. plughw:CARD=CODEC,DEV=0)
  ...
  int client_timeout_ms =
      10000;               // drop clients that have been silent this long
...
// Captures audio from an ALSA device, encodes it with Opus and fans it out as
// UDP datagrams to every subscribed client. There is no configured target:
// clients subscribe by sending any datagram to the bound port and stay
// subscribed by continuing to send (a periodic keepalive); silent clients are
// dropped. Like the other servers it runs its own worker thread; update() on
// the main loop is a no-op, so the streaming cadence is never coupled to the
// main loop.
