...
std::vector<uint8_t> cw_daemon::to_winkeyer(std::vector<uint8_t> input,
  ...
  if (buffer[0] != 27) {
    ...
    while (*ptr > 0) {
      switch (*ptr) {
        case '-': {
          ...
          //                    std::cout << "- pushing buffered speed to " <<
          //                    (int) current_speed << std::endl;
    ...
    switch (buffer[1]) {
      case '2': {
        ...
        // std::cout << "- setting speed to " << static_cast<int>(speed) <<
        // std::endl;
