...
//
// Receives timestamped key edges (see remote_key_protocol.h) over UDP and replays them onto the
// rig's serial control lines (DTR = key, RTS = PTT, as in cwdaemon_server's key_interface) behind a
// fixed playout delay that absorbs network jitter. A dedicated real-time thread schedules each edge
// with clock_nanosleep(TIMER_ABSTIME) so element timing does not depend on the 2 ms main loop.
//
// Unlike cwdaemon_server this path runs no keyer state machine: the operator's client forms the
// elements (iambic logic + sidetone live there, on jitter-free local input) and streams finished
// edges. A straight key is just the degenerate case of raw edges.
//
...
struct remote_key_config {
    ...
    uint32_t    playout_ms      = 150;  // jitter-buffer depth: how far the rig lags the operator
    uint32_t    silence_ms      = 250;  // force key-up if no packet (incl. keepalive) for this long
    uint32_t    max_key_down_ms = 5000; // hard watchdog: never hold the key down longer than this
...
private:
    // One scheduled edge, already converted to the local CLOCK_MONOTONIC timeline.
    ...
    uint64_t anchor_src_us    = 0;   // operator timestamp of the first edge this session
    ...
    std::atomic<uint64_t> last_rx_us{0};      // local time of the last received datagram
