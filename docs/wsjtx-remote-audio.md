...
| ROC: no audio at all on `rig_rx_roc` after a network change | ROC pushes to a **fixed receiver IP**. If the workstation's address changes (VPN drops → LAN, or a new DHCP/VPN lease), the rig host's `remote.ip` is stale and packets go nowhere. Verify on the rig host (`tcpdump -ni any 'udp and dst port 10001'` shows it sending) and on the workstation (`tcpdump` shows **nothing** arriving). Fix: set the rig host's `remote.ip` to the workstation's current address and restart PipeWire. (TX is unaffected — it targets the rig host, whose IP is stable.) |
